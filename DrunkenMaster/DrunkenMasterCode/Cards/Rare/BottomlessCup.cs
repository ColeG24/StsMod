...
public class BottomlessCup() : DrunkenMasterCard(2, CardType.Power, CardRarity.Rare, TargetType.Self)
{
    ...
    protected override IEnumerable<IHoverTip> ExtraHoverTips => [HoverTipFactory.FromPower<BottomlessCupPower>()];

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        NPowerUpVfx.CreateNormal(Owner.Creature);
        await PowerCmd.Apply<BottomlessCupPower>(choiceContext, Owner.Creature, DynamicVars[nameof(BottomlessCupPower)].BaseValue, Owner.Creature, this);
    }
